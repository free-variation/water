...
int build_set_from_values(Interpreter *interp, const Val *values, int count) {
	...
	if (count > 0) {
		lf_qsort_r(set->items, (size_t)count, sizeof(Val), interp, val_cmp_qsort);
...
void p_sort(Interpreter *interp) {
	...
	if (source->len > 0)
		lf_qsort_r(sorted->items, (size_t)source->len, sizeof(Val), interp, val_cmp_qsort);
	if (interp->error_flag) return;
...
void p_group_by(Interpreter *interp) {
	...
	for (int i = 0; i < result->len; i++) {
		...
		Object *bag_obj = OBJECT_AT(bag);
		lf_qsort_r(bag_obj->items, (size_t)bag_obj->len, sizeof(Val), interp, row_cmp);
		int unique = 0;
