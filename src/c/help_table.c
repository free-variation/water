...
const HelpEntry help_entries[] = {
	...
	{ "lt", "( a b -- bool ) or ( m/arr x -- m )", "less-than; element-wise 1/0 mask on matrix operands (scalar broadcast) and on array operands (val_cmp per element, n×1)", "3 (float)", "matrix 1m(r×c)", "same; matrix O(r×c)", 4 },
...
const int help_entry_count = 540;
