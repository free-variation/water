...
- **Regression trees** — `fit-tree` grows a CART regression tree over a features frame and a numeric response: numeric columns split at a midpoint threshold, array columns are native categoricals split on a mean-ordered subset. It returns the tree as a nested frame — `:prediction` and `:n_rows` at every node, `:feature` with `:threshold` or `:categories` at internal nodes, optional per-leaf `:responses` — and takes a params frame (`:max-depth`, `:min-samples`, `:store-leaf-responses`). `pfit-tree` is the parallel form, growing independent subtrees across cores into a byte-identical tree. `fit-tree-hist` is the histogram form: numeric columns bin once into at most 256 uniform-width codes (`:max-bins`) and split thresholds are bin edges — approximate splits at a fraction of the cost, built for boosting-depth trees over large data.
