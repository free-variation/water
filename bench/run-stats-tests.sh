#!/usr/bin/env bash
#
# Run the heavy statistics benchmarks (LF-only; serial vs parallel bootstrap).
# Ensures the synthetic dataset exists (generated once via gen-logistic-sim.l4),
# then runs each bench/stats/*.l4 benchmark with wall-clock time and exit status.
#
...
ensure_sim() {
	...
	( cd "$root" && "$bin" -b < "$here/stats/gen-logistic-sim.l4" ) >&2 || { log "  generation failed"; return 1; }
}
