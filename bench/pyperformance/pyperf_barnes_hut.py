...
def run(loops, iterations, theta):
    width = 1000
    height = 800
    particles = create_deterministic_galaxy(DEFAULT_PARTICLES, width / 2, height / 2)
    ie = calculate_system_energy(particles)
    t0 = _t.perf_counter()
    fe = ie
    for _ in range(loops):
        for _ in range(iterations):
            advance_system(particles, theta, TIME_STEP, width, height)
        fe = calculate_system_energy(particles)
    return _t.perf_counter() - t0, ie, fe
...
if __name__ == "__main__":
    ...
    dt, ie, fe = run(loops, iterations, DEFAULT_THETA)
    print(f"initial energy: {ie!r}")
    print(f"final energy: {fe!r}")
    print(f"elapsed: {dt:.6f} s ({loops}x{iterations})")
