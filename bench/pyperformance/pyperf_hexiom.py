...
def run(loops, level):
    ...
    sig = []
    orig = solved
    def cap(pos, output, verbose=False):
        r = orig(pos, output, verbose)
        if r == SOLVED:
            sig.append(''.join(str(pos.done[k][0]) for k in range(pos.hex.count)))
        return r
    globals()['solved'] = cap
    t0 = _t.perf_counter()
    for _ in range(loops):
        sig.clear()
        pos = read_file(board)
        solve_step_first(pos)
    dt = _t.perf_counter() - t0
    globals()['solved'] = orig
    return dt, (sig[0] if sig else "NONE")


def solve_step_first(pos):
    from_pos = pos
    p = from_pos.clone()
    while constraint_pass(p):
        pass
    return _ss(p)


def _ss(pos):
    moves = find_moves(pos, Done.FIRST_STRATEGY, DESCENDING)
    if len(moves) == 0:
        return solved(pos, None)
    for move in moves:
        new_pos = pos.clone()
        play_move(new_pos, move)
        while constraint_pass(new_pos, move[0]):
            pass
        cur = solved(new_pos, None)
        ret = cur if cur != OPEN else _ss(new_pos)
        if ret == SOLVED:
            return SOLVED
    return IMPOSSIBLE
...
if __name__ == "__main__":
    ...
    print(f"elapsed: {dt:.6f} s ({loops} solves)")
