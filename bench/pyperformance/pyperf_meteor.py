...
def run(loops, board, pieces, solve_arg, fps, se_nh):
    t0 = _t.perf_counter()
    solutions = []
...
if __name__ == "__main__":
    ...
    dt, solutions = run(loops, board, pieces, SOLVE_ARG, fps, se_nh)
    ok = (solutions == SOLUTIONS)
    print(f"solutions: {len(solutions)}  match: {1 if ok else 0}")
    print(f"first: {solutions[0]}")
    print(f"last: {solutions[-1]}")
