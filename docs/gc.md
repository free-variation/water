...
This is a primer on how logicforth reclaims memory. By the end you should
understand:
...
logicforth uses *mark-and-sweep*, one of the oldest collection algorithms. It
runs in two phases.
...
live, everything else dead. In logicforth the roots come from five places:
...
— costs an O(n) clear pass over the whole heap each time. logicforth instead uses
...
logicforth doesn't slide live values down to close them, because a value is named
