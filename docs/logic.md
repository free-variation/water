...
rather than fixed values — and on how logicforth provides it. By the end you
should understand:
...
`src/c/core.c`; the fact-database layer is in the standard library. logicforth's
...
of substitutions. logicforth provides the same operations and the same behavior by
a different route:
...
*stream* that disjunction interleaves. logicforth keeps one *mutable* substitution
...
Like most Prologs, logicforth omits the **occurs check**: unifying a variable with
