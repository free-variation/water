...
| `fit-tree-hist` | `( features y params -- tree )` | `fit-tree`'s histogram form — same inputs, same output-frame shape, same split criterion, approximate thresholds. Each numeric column is binned once into `:max-bins` (default 256, clamped [2,256]) uniform-width `uint8` codes; candidate thresholds are bin upper edges, so the chosen cut sits within one bin-width of the exact optimum. Categorical columns split exactly as `fit-tree`, their level codes serving as bins. Per node a `(sum, count)` histogram per feature is prefix-scanned for the best split; only the smaller child's histogram is accumulated (the sibling is parent minus child), the build runs across `num-cores` workers for large nodes, and the row partition is stable, keeping node windows in ascending row order for sequential gathers. `:max-depth` is clamped to 64 (the per-depth histogram stack); other params as `fit-tree`. Built for boosting-depth trees over large data; `fit-tree` remains the exact reference | features·(n + bins·nodes) | `1B` code per numeric cell + (depth+1)·features·bins histogram stack + row buffers + tree frame | O(features·n·depth / cores) |
